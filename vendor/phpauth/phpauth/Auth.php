...
class Auth
{
	...
	*/
	...
	public function login($userName, $remember = 0, $captcha = NULL)
	{
		$return['error'] = true;
	...
	*/
	...
	public function register($userName,$id, $params = Array(), $captcha = NULL, $sendmail = NULL)
	{
		$return['error'] = true;
        ...
        // Validate email
        $validateEmail = $this->validateUsername($userName);
        if ($validateEmail['error'] == 1) {
		...
		$addUser = $this->addUser($userName,$id, $params, $sendmail);
	...
	*/
    ...
    {
        $query = $this->dbh->prepare("SELECT count(*) FROM {$this->config->table_users} WHERE username = ?");
        $query->execute(array($userName));
	...
	*/
	...
	private function addUser($userName,$id, $params = array(), &$sendmail)
	{
		$return['error'] = true;
		...
		$email = htmlentities(strtolower($userName));

		if($sendmail) {
			$addRequest = $this->addRequest($uid, $userName, "activation", $sendmail);
		...
		$query = $this->dbh->prepare("UPDATE {$this->config->table_users} SET username = ?, instagramID = ? {$setParams} WHERE id = ?");

		$bindParams = array_values(array_merge(array($userName,$id), $params, array($uid)));
	...
	*/
	...
	private function validateEmail($userName) {
		...
		if (strlen($userName) < (int)$this->config->verify_email_min_length ) {
			$return['message'] = $this->lang["email_short"];
			return $return;
		} elseif (strlen($userName) > (int)$this->config->verify_email_max_length ) {
