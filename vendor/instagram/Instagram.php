...
class Instagram
{
     ...
     * @return mixed
     */
    public function getUserLikes($userMedia)
    {
        if(!empty($userMedia)) {
            $likesCount = 0;
    ...
    public function getUserComments ($userMedia){
        if(!empty($userMedia)) {
            $commentsCount = 0;
            foreach ($userMedia->data as $media) {
    ...
    public function getUserTags ($userMedia){
        if(!empty($userMedia)) {
            $tagsCount = 0;
            foreach ($userMedia->data as $media) {
                $tagsCount += count($media->tags);
            }
    ...
    protected function _makeCall($function, $auth = false, $params = null, $method = 'GET')
    {
        ...
        if ($this->_signedheader) {
            $apiCall .= (strstr($apiCall, '?') ? '&' : '?') . 'sig=' . $this->_signHeader($function, $authMethod,
                    $params);
        }
